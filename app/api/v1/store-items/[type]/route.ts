...
const getMovieChart = async (): Promise<string | null> => {
  try {
    const response = await axios.get(
      "https://api.themoviedb.org/3/movie/popular?language=en-US$country=us&page=1",
      {
...
const getHeadlines = async (type: string): Promise<string[]> => {
  ...
  const baseUrl = `https://gnews.io/api/v4/top-headlines?lang=en&country=us&category=${category}&apikey=${GNEWS_API_KEY}`;

  try {
    const response = await axios.get(baseUrl);
    console.log("-----asqweqwe----,", response.data);
...
const getNews = async (title: string): Promise<string | null> => {
  ...
  const baseUrl = `https://gnews.io/api/v4/search?lang=en&country=us&q=${title}&apikey=${GNEWS_API_KEY}`;
